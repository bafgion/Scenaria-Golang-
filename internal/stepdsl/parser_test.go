...
func TestParse(t *testing.T) {
	tests := []struct {
		...
		{
			name:   "assert-enabled",
			step:   gherkin.Step{Line: 5, Text: `проверяю что доступно "#submit"`},
			kind:   "assert-enabled",
			value1: "#submit",
		},
		{
			name:   "assert-disabled",
			step:   gherkin.Step{Line: 5, Text: `проверяю что недоступно "#submit"`},
			kind:   "assert-disabled",
			value1: "#submit",
		},
		{
			name:   "wait-enabled",
			step:   gherkin.Step{Line: 5, Text: `жду пока доступно "#pay"`},
			kind:   "wait-enabled",
			value1: "#pay",
		},
		{
			name:   "assert-text-regex",
			step:   gherkin.Step{Line: 6, Text: `проверяю что текст соответствует regex "Pay.*\d+" в "#pay"`},
		...
		},
		{
			...
			step:   gherkin.Step{Line: 6, Text: `проверяю что выбрано ".tile.active"`},
			kind:   "assert-selected",
			value1: ".tile.active",
		},
		{
			name:   "remember-number",
			step:   gherkin.Step{Line: 6, Text: `запоминаю число из "#total" как "total"`},
			kind:   "remember-number",
			value1: "#total",
			value2: "total",
		},
		{
			name:   "assert-var-contains",
			step:   gherkin.Step{Line: 6, Text: `проверяю что "{{a}}" содержит "{{b}}"`},
