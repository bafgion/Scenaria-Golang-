...
var stepPatterns = []struct {
	...
	{regexp.MustCompile(`(?i)^запоминаю число из "` + quoted + `" как "` + quoted + `"$`), "remember-number", two("remember-number")},
	...
	{regexp.MustCompile(`(?i)^проверяю что доступно "` + quoted + `"$`), "assert-enabled", one("assert-enabled")},
	{regexp.MustCompile(`(?i)^проверяю что недоступно "` + quoted + `"$`), "assert-disabled", one("assert-disabled")},
	{regexp.MustCompile(`(?i)^проверяю что выбрано "` + quoted + `"$`), "assert-selected", one("assert-selected")},
	...
	{regexp.MustCompile(`(?i)^проверяю что текст соответствует regex "` + quoted + `" в "` + quoted + `"$`), "assert-text-regex", two("assert-text-regex")},
	{regexp.MustCompile(`(?i)^проверяю что "` + quoted + `" содержит "` + quoted + `"$`), "assert-var-contains", two("assert-var-contains")},
	{regexp.MustCompile(`(?i)^проверяю что "` + quoted + `" равно "` + quoted + `"$`), "assert-var-equals", two("assert-var-equals")},
	...
	{regexp.MustCompile(`(?i)^ожидаю адрес "` + quoted + `"$`), "wait-url", one("wait-url")},
	...
	{regexp.MustCompile(`(?i)^жду пока доступно "` + quoted + `"$`), "wait-enabled", one("wait-enabled")},
	{regexp.MustCompile(`(?i)^жду пока недоступно "` + quoted + `"$`), "wait-disabled", one("wait-disabled")},
