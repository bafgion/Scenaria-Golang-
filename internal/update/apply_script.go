...
func setupUpdateScript(setupPath, installDir string, parentPID int) string {
	setup := batchQuote(filepath.Clean(setupPath))
	target := batchQuote(filepath.Clean(installDir))
	gui := batchQuote(filepath.Join(filepath.Clean(installDir), brand.GUIExeName))
	pid := strconv.Itoa(parentPID)
	lines := []string{
		...
		"ping -n 2 127.0.0.1 >nul",
		":run_setup",
		`start "" /WAIT ` + setup + ` /VERYSILENT /SUPPRESSMSGBOXES /CLOSEAPPLICATIONS /NORESTART`,
		"set RC=%ERRORLEVEL%",
		"if %RC% NEQ 0 exit /b %RC%",
		`if exist ` + gui + ` start "" /D ` + target + ` ` + gui,
		"endlocal",
		"del \"%~f0\" >nul 2>&1",
		"exit /b 0",
	}
