...
| Шаг | Назначение |
|-----|------------|
| `вижу "selector"` | Элемент **виден** на странице |
| `не вижу "selector"` | Элемент **скрыт** или отсутствует |
| `проверяю что доступно "selector"` | Элемент виден и **можно нажать** (не `disabled`, не перекрыт) |
| `проверяю что недоступно "selector"` | Элемент виден, но **нельзя нажать** (`disabled`, `aria-disabled` и т.п.) |
| `проверяю текст "…" в "selector"` | Текст внутри элемента |
| `проверяю url "…"` | Точный или частичный URL |

Шаг `вижу` **не** проверяет доступность кнопки: неактивная кнопка на экране пройдёт `вижу`, но упадёт на `нажимаю`. Для явной проверки используйте `проверяю что доступно` / `проверяю что недоступно`.

Английский DSL (`# language: en`): `I see "selector" is enabled` / `I see "selector" is disabled`.
